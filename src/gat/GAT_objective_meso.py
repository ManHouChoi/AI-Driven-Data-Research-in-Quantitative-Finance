...
CONFIG = {
    ...
    "risk_csv": os.getenv(
        ...
        os.path.join(os.getenv("FYP_PROJECT_ROOT", os.getcwd()), "data", "processed", "risk_scores_meso_annual.csv"),
    ),
    "fin_csv": os.getenv(
    ...
    "output_dir": os.getenv(
        ...
        os.path.join(os.getenv("FYP_PROJECT_ROOT", os.getcwd()), "outputs", "gat", "meso"),
    ),
    "n_trials": 50,
    "max_epochs": 200,
    "patience": 25,
...
def objective(trial: optuna.Trial, risk_df: pd.DataFrame, fin_df: pd.DataFrame) -> float:
    ...
    if trial.number == 0:
        ...
        diagnostics.to_csv(
            os.path.join(diagnostics_dir, "graph_diagnostics_meso_initial_trial.csv"),
            index=False,
        )
    ...
    if trial.number % 10 == 0:
        ...
        plt.title(f"Meso Optuna Trial {trial.number}: Loss Convergence")
        plt.xlabel("Epoch")
...
def main() -> None:
    ...
    print("Loading meso datasets...")
    if not os.path.exists(CONFIG["risk_csv"]):
        raise FileNotFoundError(
            f"Meso risk score file not found: {CONFIG['risk_csv']}. "
            "Set FYP_RISK_SCORES_MESO_CSV or place the file under data/processed/."
        )
    ...
    precheck_diag.to_csv(
        os.path.join(output_dir, "graph_diagnostics_meso_tau_085_precheck.csv"),
        index=False,
    )

    print("Starting Meso Optuna hyperparameter optimization...")
    ...
    study = optuna.create_study(
        direction="minimize",
        study_name="ST_GAT_Meso_Optimization",
        sampler=sampler,
        pruner=pruner,
    )
    ...
    with open(os.path.join(output_dir, "optuna_best_params_meso.json"), "w") as f:
        json.dump(best_payload, f, indent=4)

    with open(os.path.join(output_dir, "optuna_best_params_meso.txt"), "w") as f:
        f.write("Best Hyperparameters from Meso Optuna Optimization\n")
        f.write("-" * 60 + "\n")
    ...
    trials_df.to_csv(os.path.join(output_dir, "optuna_trials_history_meso.csv"), index=False)

    try:
        ovm.plot_optimization_history(study)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "optuna_optimization_history_meso.png"), dpi=300)
        plt.close()

        ovm.plot_param_importances(study)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "optuna_param_importances_meso.png"), dpi=300)
        plt.close()
    except Exception as exc:
        print(f"Warning: Could not generate Optuna plots. Error: {exc}")

    print("\n" + "=" * 60)
    print(f"Meso Optuna phase complete. Outputs saved in '{output_dir}'.")
    print("=" * 60)
