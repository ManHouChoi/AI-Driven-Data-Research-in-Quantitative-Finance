...
\subsection{Firm-Level Taxonomy Validation: Case Studies}
Firm-level case studies are used as interpretability checks rather than as portfolio results. The question is whether the taxonomy records intuitive changes in disclosure emphasis as each firm's operating environment changes. Figure \ref{fig:taxonomy_case_study_macro_profiles} compares the $\theta=0.40$ Macro exposure profiles for several firms with the longest available histories in the reconstructed taxonomy panel. The case-study heat map shows that the dynamic taxonomy can represent both persistent firm identity and time variation: real-estate and gaming names concentrate in property, governance, operational-event, and market-credit families, while shifts in the heat map identify periods when disclosures move toward different risk families.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{taxonomy_case_study_macro_profiles.png}
    \caption[Firm-level taxonomy case studies]{Firm-level Macro risk exposure case studies under the $\theta=0.40$ taxonomy. Each row is a firm-year and each cell is the exposure share assigned to a Macro family.}
    \label{fig:taxonomy_case_study_macro_profiles}
\end{figure}

The Las Vegas Sands (LVS) diagnostic over 2008--2024 provides a deeper single-firm view. It is used as an interpretability check rather than as a portfolio result. The question is whether the taxonomy records intuitive changes in LVS disclosure emphasis as its operating environment changes.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{taxonomy_lvs_composition_shift.jpg}
    \caption[LVS risk-composition shift]{LVS risk-composition shift over 2008--2024. The stacked-area chart illustrates how a firm-level exposure vector can move across Meso categories over time.}
    \label{fig:lvs_composition_shift}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{taxonomy_lvs_emerging_declining_risks.jpg}
    \caption[LVS emerging and declining risks]{LVS emerging and declining risk categories between 2008 and 2024. The figure is used as a taxonomy interpretability diagnostic, not as an investment signal.}
    \label{fig:lvs_emerging_declining}
\end{figure}

The LVS figures show how the taxonomy can be audited at the firm level: emerging categories include legal, concession, brand, and business-interruption themes, while declining categories include some earlier financial-statement and fixed-obligation categories. This kind of case study helps non-expert readers understand the purpose of the taxonomy. It does not by itself prove predictive power; predictive value is evaluated later through the identity-matrix ST-GAT ablation and portfolio diagnostics.
